...
Agrega la prop `sort` a cualquier `SDColumn` para hacer esa columna ordenable. Al hacer clic en el encabezado, `SDTable` emite el evento `sort`; tu aplicacion ordena el arreglo `data` y vuelve a pasar la direccion actual (`'asc'` o `'des'`) a la columna activa para actualizar el indicador visual.
...
| `sort` | `boolean \| 'asc' \| 'des'` | - | Habilita el encabezado ordenable. Usa `true` cuando la columna esta disponible para ordenar, o `'asc'`/`'des'` para mostrar la direccion activa |
