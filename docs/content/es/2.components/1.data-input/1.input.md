...
El componente `SInputBlock` envuelve a `SInput` con una etiqueta, texto de ayuda y texto de error. Combina todas las props del componente base con `label`, `helpText` y `errorText`. Cuando se proporciona `errorText`, el estado de error se aplica automaticamente.
