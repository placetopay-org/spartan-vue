...
Este componente comparte patrones visuales con **SInput**. Consultalo para props comunes de input como `rounded`, `borderless`, `disabled` y `error`.
::
...
El componente `SInputAmountBlock` envuelve a `SInputAmount` con una etiqueta, texto de ayuda y texto de error. Combina todas las props del componente base con `label`, `helpText` y `errorText`. Cuando se proporciona `errorText`, el estado de error se aplica automaticamente.
