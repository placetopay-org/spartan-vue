...
Usa el slot `trigger` para personalizar completamente el contenido del botón del selector. El slot recibe el objeto del option seleccionado ya resuelto (matcheado contra `options` incluso cuando `modelValue` es un primitivo) y el prop `placeholder`, así puedes renderizar contenido enriquecido como banderas, iconos o etiquetas de dos líneas en el trigger.
...
El componente `SSelectorBlock` envuelve a `SSelector` con una etiqueta, texto de ayuda y texto de error. Combina todas las props del componente base con `label`, `helpText` y `errorText`. Cuando se proporciona `errorText`, el estado de error se aplica automaticamente.
...
|--------|-------|-------------|
| `trigger` | `{ option, placeholder }` | Renderizado personalizado para el botón del selector. `option` es el objeto del option seleccionado ya resuelto (o `undefined` cuando no hay valor o el valor no matchea ninguna option). |
