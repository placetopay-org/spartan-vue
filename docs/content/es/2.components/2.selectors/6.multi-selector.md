...
Este componente sigue el mismo patron de API que **SSelector**. Consultalo para el equivalente de seleccion simple con props y comportamiento similares.
::
...
Cuando `search` y `handler` estan ambos habilitados, aparece un boton de agregar que permite a los usuarios crear nuevas opciones al vuelo. El evento `add` se emite con el texto de busqueda cuando se hace clic en el boton.
...
Usa el slot `trigger` para personalizar completamente el contenido del botón del selector. El slot recibe el array de options seleccionadas ya resueltas (matcheadas contra `options` incluso cuando los items de `modelValue` son primitivos) y el prop `placeholder`, así puedes renderizar contenido enriquecido como banderas, iconos o resúmenes en el trigger.
...
| `update:options` | `(Record<string, any> \| string)[]` | Se emite cuando la lista de opciones cambia internamente |

## Slots

| Nombre | Props | Descripcion |
|--------|-------|-------------|
| `trigger` | `{ options, placeholder }` | Renderizado personalizado para el botón del selector. `options` es el array de option seleccionadas ya resueltas (matcheadas contra `options` incluso cuando los items de `modelValue` son primitivos). |
