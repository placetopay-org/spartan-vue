...
- The internal `ModalBackdropWrapper` (behind every `breakpoint`-aware modal: `SModalSide`, `SModalLeft`) registered a `matchMedia` listener it never removed — every mount leaked a MediaQueryList listener for the life of the page. It is now removed on unmount. `SCopy` likewise left its 3-second feedback timeout running after unmount; it is now cleared.
