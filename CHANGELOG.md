...
### Added
- `pnpm run status:sync` (`scripts/syncComponentStatus.js`) writes the measured coverage into the `tests` field of every entry in `componentStatus.ts`. Nobody types that number any more: `status:sync` writes it, `check:status` fails if it drifted, and both compute it from `scripts/coverage.js` so they cannot disagree. `sonar.yml` runs `check:status` too — it is the only job that produces coverage on a pull request.
