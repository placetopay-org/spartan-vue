...
- `.release-it.json`: `npm.skipChecks` stays enabled. Removing it aborted the `3.0.0-beta.17` publish with `Not authenticated with npm.`: release-it's only hard pre-flight is `npm whoami`, and the CI token cannot call it even though it publishes successfully. Nothing was mutated — the run died in `init`, before the bump, the tag and the push — which is the behaviour the removal was meant to buy in the first place. A non-blocking diagnostic step in `publish.yml` now records whether the token cannot reach `/-/whoami` or simply never reaches the `npm` that release-it spawns under `pnpm`.
