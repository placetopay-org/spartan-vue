...
### Changed
- Every `devDependency` updated to its latest version, majors included: `primevue`/`@primevue/icons` 5 (re-pinned exactly — both are inlined into `dist`), `@floating-ui/vue` 2 (bundled; the 39 SPopover/STooltip tests, including the double-offset regression guards, still pass), `vite` 8, `tailwindcss`/`@tailwindcss/vite` 4.3, `tailwind-merge` 3.6, `jsdom` 29, `@testing-library/dom` 10, `release-it` 20 (`@release-it/keep-a-changelog` 7.1 declares support for it), `prettier` 3.9, `nuxt` 4.4, `wrangler` 4.111. The published runtime is untouched: `dependencies` (`@tailwindcss/forms`, `vue-sonner`) were already current and `vue`/`vue-i18n` remain externalized peers. `typescript` is pinned to **6.0.3**, the last JS-line release — 7.x is the native port, which drops `./lib/tsc` and cannot run `vue-tsc`; `typescript-eslint` also caps at `<6.1`. TypeScript 6 requires an explicit `rootDir` for declaration emit (TS5011), now set to `./src` in `tsconfig.json` — the emitted layout is unchanged.
