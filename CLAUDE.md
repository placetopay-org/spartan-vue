...
Spartan Vue is a Vue 3 component library (64 components) by PlaceToPay, built with TailwindCSS v4 and TypeScript. Package: `@placetopay/spartan-vue`. This is a **library**, not an application.
...
- **Vue 3.5+ syntax**: Use reactive destructuring with defaults for `defineProps` (no `withDefaults`). Use same-name shorthand for bindings: `:variant` instead of `:variant="variant"`. Exception: components that must pass the whole reactive `props` object to a context/helper (SSidebar, SFilter, SInput) keep `withDefaults` — destructuring would lose the reactive object handle.
...
3. **Rollup** — Bundles the i18n expose entry (`src/expose/i18n.ts` → `dist/expose/i18n.js`)
