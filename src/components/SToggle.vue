...
<template>
  <SwitchGroup as="div" class="flex items-center justify-between">
    <Switch :modelValue="modelValue" @update:modelValue="$emit('update:modelValue', $event)"
      :class="[enabled ? 'bg-primary-600' : 'bg-gray-200', 'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out mr-4 focus:outline-none focus:ring-2 focus:ring-primary-600 focus:ring-offset-2']">
