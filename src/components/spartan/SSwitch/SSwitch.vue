    ...
    <SwitchGroup as="div" :class="['flex items-center gap-3', reverse ? 'flex-row-reverse justify-between' : '']">
        <Switch
            ...
            :class="[
                ...
                'focus:ring-spartan-primary-600 relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:ring-2 focus:ring-offset-2 focus:outline-none',
            ]"
        >
            <span
                aria-hidden="true"
            ...
            >
                <template v-if="icon || iconOff || iconOn">
                    ...
                    <span
                        ...
                        aria-hidden="true"
                    >
                        <component :is="iconOn" v-if="iconOff" class="text-spartan-primary-600 h-3 w-3" />
                        <template v-if="icon">
                            <svg
                                v-if="typeof icon === 'boolean'"
                                class="text-spartan-primary-600 h-3 w-3"
                                fill="currentColor"
                                viewBox="0 0 12 12"
                            >
                                <path
                            ...
                            <component :is="icon" v-else class="text-spartan-primary-600 h-3 w-3" />
                        </template>
                    </span>
                </template>
            </span>
