...
export const inputStyles = cva(
    `${inputStyle.root} ${inputStyle.text} ${inputStyle.placeholder} ${inputStyle.padding} w-full border-none outline-none focus:ring-0`,
    {
        variants: {
            rounded: roundedStyle,
        },
...
export const buttonStyles = cva([inputStyle.root, inputStyle.padding, 'gap-1 inline-flex items-center'], {
    variants: {
        disabled: cbv('bg-gray-50 dark:bg-gray-800 pointer-events-none'),
        error: cbv(
            `${inputStyle.border.error} ${inputStyle.ring.error}`,
            `${inputStyle.border.base} ${inputStyle.ring.base}`,
        ),
...
export const optionStyles = cva('relative text-sm flex w-full py-2 pl-3 pr-[52px] hover:bg-gray-50 dark:hover:bg-white/10', {
    variants: {
