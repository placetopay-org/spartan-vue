<script setup lang="ts">
import { computed, onMounted, ref } from 'vue';
import type { TTabItemProps } from '../types';
import { useContext } from '../api';

// Shared implementation of the chip-shaped tab items. Pill and Vetch were two
// byte-identical copies of this file that differed only in the active/inactive
// class pair; each variant now passes its palette in.
const {
    as = 'button',
    path,
    active,
    regex,
    activeClass,
    inactiveClass,
} = defineProps<TTabItemProps & { activeClass: string; inactiveClass: string }>();

const context = useContext('STabItem');

const el = ref<HTMLElement>();
const vPath = ref(path || '');
const vRegex = computed(() => regex || new RegExp(`^${vPath.value}$`));
const vActive = computed(() => active || vRegex.value.test(context.modelValue));

onMounted(() => {
    const elInnerText = el.value?.innerText;
    if (!vPath.value && elInnerText) vPath.value = elInnerText;
});
</script>

<template>
    <component
        :is="as"
        v-if="as"
        ref="el"
        :type="as === 'button' ? 'button' : undefined"
        :class="[
            vActive ? activeClass : inactiveClass,
            'group inline-flex items-center rounded-md px-3 py-2 text-sm font-medium',
            $props.class,
        ]"
        :aria-current="vActive ? 'page' : undefined"
        @click="() => context.updateModelValue(vPath)"
    >
        <component
            :is="icon"
            v-if="icon"
            :class="[
                vActive
                    ? 'text-spartan-primary-500'
                    : 'text-gray-400 group-hover:text-gray-500 dark:text-gray-500 dark:group-hover:text-gray-300',
                'mr-2 -ml-0.5 h-5 w-5',
            ]"
            aria-hidden="true"
        />
        <slot />
    </component>
</template>
